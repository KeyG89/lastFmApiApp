...
- `src/lastfm_app/shazam.py`: dedicated Shazam SQLite database, Shazam/RapidAPI catalog search, Spotify playlist API import, fallback CSV/JSON import, Last.fm/Spotify linking, and generated playlist plans.
...
Shazam stays in a separate database because it is a discovery inbox rather than a listening-history source. Cross-database links use normalized artist/title keys first, then Spotify IDs when available. The app does not assume a public Apple web API for the user's full Shazam history; it supports Shazam/RapidAPI catalog lookup and Spotify playlist import for synced Shazam libraries.
